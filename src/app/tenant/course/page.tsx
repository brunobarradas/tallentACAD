...
type Props = {
  params: Promise<{ tenant: string; course: string }>
}

export default async function CoursePage({ params }: Props) {
  const { tenant: tenantSlug, course: courseSlug } = await params
    ...
    <main className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <a href={`/${tenantSlug}`} className="flex items-center gap-2 text-[#1e3a5f] hover:opacity-80">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="mt-10">
          <h2 className="text-xl font-bold text-[#1e3a5f] mb-4">Conteudo do curso</h2>
                  ...
                  <div className="flex-1">
                    ...
                    {!isUnlocked && (
                      <p className="text-xs text-gray-400 mt-0.5">
                        Disponivel no dia {lesson.unlock_after_days} do curso
                      </p>
                    )}
                  </div>
                  <span className="text-gray-400 text-sm">
                    {isUnlocked ? 'Ver' : 'Bloqueado'}
                  </span>
                </div>
              )
            })}
